...
const ANNOUNCEMENTS_CACHE_KEY = 'global:announcements:feed:v1';

export async function invalidateAnnouncementsCache() {
  await invalidateCache(ANNOUNCEMENTS_CACHE_KEY);
}
 ...
 * Uses Shared Cache to avoid D1 query storms during polling.
 */
export async function getAnnouncementsUpdates() {
  ...
  try {
    const baseData = await getOrSetCache<{
      announcements: any[];
      comments: any[];
      rawReactions: { announcement_id: string; emoji: string; user_id: string }[];
    }>(
      ...
      async () => {
        ...
        if (annList.length === 0) {
          return { announcements: [], comments: [], rawReactions: [] };
        }
        ...
        return {
          ...
          rawReactions: (reactionsRaw.results || []) as any[],
        };
      },
      30 // 30 seconds TTL for polling
    );

    if (!baseData || baseData.announcements.length === 0) {
      return { announcements: [], comments: [], reactions: [] };
    }

    // Map aggregated reactions dynamically for current user
    const reactionsMap: Record<string, { announcement_id: string; emoji: string; count: number; user_reacted: number }> = {};
    for (const r of baseData.rawReactions) {
      const key = `${r.announcement_id}:${r.emoji}`;
      if (!reactionsMap[key]) {
        reactionsMap[key] = {
      ...
      reactionsMap[key].count += 1;
      if (r.user_id === session.userId) {
        reactionsMap[key].user_reacted = 1;
      }
    }
