...
export async function createAssessmentTask(workspaceId: string, formData: FormData) {
  ...
  if (deadlineStr) {
    ...
    if (deadline <= Date.now()) {
      return { success: false, error: 'Tenggat waktu harus di masa mendatang (tidak boleh waktu yang sudah lewat).' };
    }
  }
  ...
  try {
    ...
    await db
      .prepare(`
        INSERT INTO tasks (id, workspace_id, project_id, title, description, status, priority, task_type, deadline, created_by, created_at)
...
/**
 * Coordinator approves mentor's assessment draft -> changes task status to APPROVED & mass-assigns all OJT.
 */
export async function approveAssessmentTask(taskId: string, workspaceId: string, execType: string = 'DESIGNER') {
  const session = await getSession();
  if (!session) return { success: false, error: 'Unauthorized' };
  ...
  const task = await db
    .prepare('SELECT id, deadline, status FROM tasks WHERE id = ? AND workspace_id = ?')
    .bind(taskId, workspaceId)
    .first() as { id: string; deadline: number | null; status: string } | null;

  if (!task) return { success: false, error: 'Task assessment tidak ditemukan.' };

  try {
    // 1. Update task status to APPROVED (Published)
    await db
...
/**
 * Coordinator soft-deletes an assessment task and associated assignments.
 */
export async function deleteAssessmentTask(taskId: string, workspaceId: string) {
  ...
  if (!isCoordinator) {
    return { success: false, error: 'Hanya Koordinator yang dapat menghapus tugas assessment.' };
  }

  try {
    await db
      .prepare("UPDATE tasks SET status = 'DELETED' WHERE id = ? AND workspace_id = ?")
      .bind(taskId, workspaceId)
      .run();
