...
export default function HeaderMessengerButton() {
  ...
  const [filter, setFilter] = useState<'ALL' | 'UNREAD' | 'REQUESTS' | 'FRIENDS'>('ALL');
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-12 w-[340px] sm:w-[380px] bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150 text-left">
          {/* Header */}
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none font-bold text-xs">
              {(
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-xl text-[11px] transition-all shrink-0 ${
                    filter === tab.id
          ...
          <div className="max-h-[360px] overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/60 scrollbar-thin">
            {loading ? (
              ...
              <div className="py-10 text-center space-y-2 text-zinc-400 p-4">
                ...
                <p className="text-xs font-bold text-zinc-600 dark:text-zinc-300">Belum Ada Percakapan</p>
                <p className="text-[10px] text-zinc-400">
                  Klik profil user mana saja di sistem dan pilih &quot;Personal Chat&quot; untuk memulai.
                </p>
              </div>
            ) : (
              filteredList.map((c) => (
