...
export default async function ProfilePage({
  ...
  const allAchievementHistory = await getAchievementHistoryAction('ALL', targetUserId);
      ...
      {profile?.user_type !== 'STAFF' && (
        <div className={`${card} p-4 sm:p-6 border-purple-500/20 bg-gradient-to-br from-purple-500/[0.03] to-indigo-500/[0.03]`}>
          ...
          <ProfileAchievementsSection
            targetUserId={targetUserId}
            userName={profile?.name || session.name || 'User'}
            userAvatar={avatarSrc}
            userAchievements={userAchievements}
            allAchievementHistory={allAchievementHistory}
          />
