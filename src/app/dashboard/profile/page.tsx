...
import { getUserAchievementStatsAction } from '@/modules/achievements/actions';
...
export default async function ProfilePage({
  ...
  const userAchievements = await getUserAchievementStatsAction(targetUserId);
    ...
    <div className="space-y-6">
      ...
      {profile?.user_type !== 'STAFF' && (
        <div className={`${card} p-4 sm:p-6 border-purple-500/20 bg-gradient-to-br from-purple-500/[0.03] to-indigo-500/[0.03]`}>
          ...
          {/* Gelar Keahlian & Achievement History Section */}
          <div className="pt-4 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <Link
                href={`/dashboard/achievements?userId=${targetUserId}`}
            ...
            {userAchievements.length === 0 ? (
              <div className="p-3 rounded-2xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800 text-xs text-zinc-400 font-medium">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {userAchievements.map((ach) => (
                  <Link
                    ...
                    href={`/dashboard/achievements?category=${ach.category}&userId=${targetUserId}`}
                    className="p-3.5 rounded-2xl border bg-white/80 dark:bg-zinc-900/80 hover:border-purple-500/50 transition-all flex items-center justify-between gap-3 shadow-xs group cursor-pointer"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium mt-0.5">
                          <span className="font-bold text-purple-600 dark:text-purple-400">{ach.totalCount}× Champion</span> • Last: {new Date(ach.lastEarnedAt * 1000).toLocaleDateString('id-ID', { month: 'short', day: 'numeric', year: 'numeric' })}
                        </p>
                      </div>
                    </div>
